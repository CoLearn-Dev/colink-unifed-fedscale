...
def run_server(cl: CL.CoLink, param: bytes, participants: List[CL.Participant]):
    ...
    process = subprocess.Popen(f'FEDSCALE_HOME=$(pwd)/FedScale && echo $FEDSCALE_HOME && kill -9 1136899 1150863 1150866 && ps aux | grep feds',shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
